...
//! `tyutool_core::plugins::beken::driver` instead of one particular chip.
//! The concrete chip is named in the message itself via [`ChipSpec::name`].
...
use super::flash_table::FlashParams;
use super::frame::ProtocolError;
use super::ops;
use super::transport::{IoTransport, SerialIo, Transport};

/// Shared implementation for all Beken-family chips.
///
...
pub(crate) fn run_beken(
    ...
    log::info!(
        "Beken plugin starting: chip={}, port={}, mode={:?}",
        chip.name(),
        job.port,
        job.mode
    );
...
fn run_beken_on_transport<T: IoTransport>(
    job: &FlashJob,
...
fn run_flash_mode<T: IoTransport>(
    job: &FlashJob,
    transport: &mut Transport<'_, T>,
    chip: &dyn ChipSpec,
    ...
    for (i, seg) in segments.iter().enumerate() {
        ...
        let erase_start = base_addr & !(ops::SECTOR_SIZE_PUB - 1);
        let erase_end = (end_addr + ops::SECTOR_SIZE_PUB - 1) & !(ops::SECTOR_SIZE_PUB - 1);
...
fn run_erase_mode<T: IoTransport>(
    job: &FlashJob,
    transport: &mut Transport<'_, T>,
    chip: &dyn ChipSpec,
    flash_params: &FlashParams,
    ...
    const SECTOR: u32 = ops::SECTOR_SIZE_PUB;
...
fn run_read_mode<T: IoTransport>(
    job: &FlashJob,
    transport: &mut Transport<'_, T>,
    chip: &dyn ChipSpec,
    flash_params: &FlashParams,
...
fn to_flash_err(e: ProtocolError) -> FlashError {
    match e {
...
#[cfg(test)]
mod tests {
    use super::super::chip::Bk7231nSpec;

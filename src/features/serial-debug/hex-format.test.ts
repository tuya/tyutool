...
import { formatHexDump, parseHexInput } from './hex-format';
...
describe('parseHexInput', () => {
  it('parses whitespace-separated pairs', () => {
    expect(Array.from(parseHexInput('AA BB cc').bytes)).toEqual([0xaa, 0xbb, 0xcc]);
  });

  it('parses a contiguous hex string', () => {
    expect(Array.from(parseHexInput('aabbcc').bytes)).toEqual([0xaa, 0xbb, 0xcc]);
  });

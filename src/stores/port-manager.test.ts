...
describe("port-manager", () => {
  ...
  it("asks the current owner to release and respects approval", async () => {
    ...
    await pm.acquire(
      makeClaim("serial-debug", "/dev/ttyUSB0", {
        onReleaseRequest,
        onReleased,
      }),
    );
  ...
  it("waits for async onReleased to finish before swapping owners", async () => {
    const pm = usePortManagerStore();
    ...
    const acquiring = pm.acquire(makeClaim("flash", "/dev/ttyUSB0"));

    await Promise.resolve();
    expect(pm.currentOwner("/dev/ttyUSB0")).toBe("serial-debug");

    finishRelease?.();
    await expect(acquiring).resolves.toBe("ok");
    expect(pm.currentOwner("/dev/ttyUSB0")).toBe("flash");
  });
  ...
  it("when two concurrent acquires pass onReleaseRequest, only the first wins", async () => {
    ...
    await pm.acquire(
      makeClaim("current", "/dev/ttyUSB0", { onReleaseRequest }),
    );

...
- 应用版本的 Gitee 镜像只保留 `latest.json` 和 `release.json`，不上传 GUI/CLI 可执行文件；Gitee 固件镜像（`auth-firmware`、`ram-loader`）由各自工作流覆盖上传。客户端更新逻辑只使用 `url` 字段；`url_github` / `url_tuya` 镜像字段供外部系统读取，不生成 `url_gitee`。

...
| 四 | `ReplayIo`：录制真实串口字节流，回放做协议基线 | ✅ 已完成，见下 |
...
## 阶段四的结果：录制回放，协议终于有了真实基线

前三个阶段的假件都只能回答「驱动是否符合**我们的理解**」——响应是人编的。录制回放堆
上一层：字节来自真实芯片。

### 组件

- **`RecordIo<T>`**（`record-io` feature）包住活的 transport，把每一次调用追写到
  `TYUTOOL_RECORD_IO` 指定的文件。
- **`ReplayIo`**（`#[cfg(test)]`）把录制回放给驱动，并校验它还是问同样的问题。
- **线格式**是每行一个操作的十六进制文本，不是二进制——fixture 的 diff 要能看。

### 实测环境

T5AI 开发板，CH342 双口 USB 串口（1a86:55d2，A 口 COM34，序列号 5AAE168564），
Flash ID `0x001765c8`，921600 波特。

- 录制：`read -s 0x0 -l 0x1000` → **163 个操作、9021 字节**，包含复位、握手、读 flash id、
  读 4 KiB、重启全程。
- 同时完整烧录了 `auth-firmware-t5ai-1.1.1.bin`（2.36 MiB / 50.2 秒），并回读前 4 KiB 与
  固件逐字节比对一致——本分支的改动没碰坏真实烧录路径。

### 一个必须做的设计判断：读取不严格，写入严格

首次回放在第 14 个操作就分叉了：录制里那里是 `ClearBuffers`，回放里驱动又读了一次。

原因不是 bug：`recv_frame` 的循环由**真实时间**截止。回放时读取瞬时返回，同一个循环就多
转了几圈、多问了几次。

判断：**读取是一个问句，沉默是合法的回答。** 录制记的是设备「说了什么」；回放时多问一次听到
沉默，恰恰是忠实的。所以 `ReplayIo` 在下一个录制操作不是读取时返回超时而**不消耗操作**。

严格性全部放在**写入**一侧——协议断言本来就在那里。跳过或重排了交互的驱动，下一次写入
就会被抓住；停得太早的驱动被末尾的 `remaining() == 0` 抓住。

### 金标准做过阴性验证

一个不会失败的金标准等于没有。把 fixture 里一条写入改掉**一个字节**（`...0100` → `...0199`），
测试如期失败，并直接指出位置与两边的字节串：

```
recording diverged at op 6: the driver wrote 01e0fc0100 but the recording has 01e0fc0199
```

### fixture 里的设备内容不含秘密

录制会把设备真实返回的字节写进文件。首次录制拿到的是烧录**之前**的旧固件内容，我不知道
里面有什么，所以烧录完之后**重录了一次**：现在 fixture 里的 4 KiB 就是那份公开固件的头部，
已逐字节核对。

**绝不能录制 authorize 流程并提交**——那段流量是明文凭据。这一条写在 feature 注释、
模块注释和 fixture 头部三处。

### 代价，说清楚

- **快照会过期。** 故意改协议就得拿板子重录，**不允许手改 fixture 让测试变绿**。
- **只盖了读取路径。** 写入路径（擦除 + 写 + CRC）没盖：一次 2.36 MiB 烧录的录制约 5 MB
  十六进制文本，不适合进仓库。要盖它得先决定一个方案（小分区写入、或压缩存储），
  不是随手能加的。
- **只盖了 Beken 系。** ESP 用 `open_native()` 把真实句柄交给 espflash，包不上（同前文）。
